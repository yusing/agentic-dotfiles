...
Completion: the user has that command. They run it. The script uses Bun’s built-in `Bun.markdown.ansi` renderer in their terminal and writes `~/.cache/session-usage/<thread-id>.md`.

Terminal rendering requires Bun with `Bun.markdown.ansi` support. If rendering is unavailable, the saved Markdown report remains available. `--markdown` prints Markdown to stdout; `--no-display` saves the report without rendering.
